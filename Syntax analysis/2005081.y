...
ofstream tree, logfile,errorfile;
extern int line_no;
...
%token <ysymbol> IF ELSE FOR WHILE DO BREAK RETURN SWITCH CASE DEFAULT CONTINUE PRINTLN ADDOP INCOP DECOP RELOP ASSIGNOP LOGICOP BITOP NOT LPAREN RPAREN LCURL RCURL LSQUARE RSQUARE COMMA SEMICOLON INT CHAR FLOAT DOUBLE VOID CONST_INT CONST_FLOAT ID MULOP
...
func_definition : type_specifier ID LPAREN parameter_list RPAREN{ insert_func_def($2->name, $1->data_type, $4->Identifiers_list);}
 compound_statement {
		...
		| type_specifier ID LPAREN RPAREN{insert_func_def($2->name, $1->data_type, {});} compound_statement {
